...
> PageTemplateResource get_campaign_templates_using_get(size=size, page=page, order=order)
...
> PageCampaignResource get_campaigns_using_get(filter_active=filter_active, size=size, page=page, order=order)
