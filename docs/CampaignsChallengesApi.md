...
try: 
    # Retrieve a list of challenges
    api_response = api_instance.get_challenges(filter_template=filter_template, filter_active_campaign=filter_active_campaign, filter_start_date=filter_start_date, filter_end_date=filter_end_date, size=size, page=page, order=order)
    pprint(api_response)
except ApiException as e:
