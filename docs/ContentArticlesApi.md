...
> PageResourceTemplateResource get_article_templates_using_get(size=size, page=page, order=order)
...
> PageResourceArticleResource get_articles_using_get(filter_category=filter_category, filter_tagset=filter_tagset, filter_title=filter_title, size=size, page=page, order=order)
