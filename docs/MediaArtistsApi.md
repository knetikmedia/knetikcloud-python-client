...
> PageTemplateResource get_artist_templates_using_get(size=size, page=page, order=order)
...
> PageArtistResource search_artists_using_get(filter_artists_by_name=filter_artists_by_name, size=size, page=page, order=order)
