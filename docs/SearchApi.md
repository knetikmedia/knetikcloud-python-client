...
# **search_using_post**
> PageMapstringobject search_using_post(type, query=query, size=size, page=page, order=order)
...
try: 
    # Search an index
    api_response = api_instance.search_using_post(type, query=query, size=size, page=page, order=order)
    pprint(api_response)
except ApiException as e:
