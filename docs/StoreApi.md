...
> PageResourceStoreItemTemplateResource get_item_templates_using_get(size=size, page=page, order=order)
...
# **get_using_get2**
> PageResourceStoreItem get_using_get2(limit=limit, page=page, use_catalog=use_catalog, ignore_location=ignore_location, in_stock_only=in_stock_only)
...
try: 
    # Get a listing of store items
    api_response = api_instance.get_using_get2(limit=limit, page=page, use_catalog=use_catalog, ignore_location=ignore_location, in_stock_only=in_stock_only)
    pprint(api_response)
except ApiException as e:
