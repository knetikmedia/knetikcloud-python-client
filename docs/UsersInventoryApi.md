...
[**grant_entitlement_using_post**](UsersInventoryApi.md#grant_entitlement_using_post) | **POST** /users/{user_id}/entitlements/ | Grant an entitlement
...
> PageResourceEntitlementItem get_currencies_using_get1(size=size, page=page, order=order)
...
> PageResourceItemTemplateResource get_entitlement_templates_using_get(size=size, page=page, order=order)
...
> PageResourceUserInventoryResource get_inventory_list_using_get(inactive=inactive, size=size, page=page, filter_item_name=filter_item_name, filter_username=filter_username, filter_group=filter_group, filter_date=filter_date)
...
> PageResourceUserInventoryResource get_user_inventory_list_using_get(id, inactive=inactive, size=size, page=page, filter_item_name=filter_item_name, filter_min_date=filter_min_date, filter_max_date=filter_max_date)
...
> PageResourceUserItemLogResource get_user_inventory_log_using_get(user_id, id, size=size, page=page)
...
# **grant_entitlement_using_post**
> grant_entitlement_using_post(user_id, grant_request)
...
try: 
    # Grant an entitlement
    api_instance.grant_entitlement_using_post(user_id, grant_request)
except ApiException as e:
    print("Exception when calling UsersInventoryApi->grant_entitlement_using_post: %s\n" % e)
```
