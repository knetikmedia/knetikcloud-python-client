...
class GamificationAchievementsApi(object):
    ...
    def update_achievement_progress(self, user_id, achievement_name, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.update_achievement_progress(user_id, achievement_name, callback=callback_function)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.update_achievement_progress_with_http_info(user_id, achievement_name, **kwargs)
        else:
            (data) = self.update_achievement_progress_with_http_info(user_id, achievement_name, **kwargs)
            return data

    def update_achievement_progress_with_http_info(self, user_id, achievement_name, **kwargs):
        """
        Update or create an achievement progress record for a user
        If no progress record yet exists for the user, it will be created. Otherwise it will be updated. If progress meets or exceeds the achievement's max_value it will be marked as earned and a BRE event will be triggered for the <code>BreAchievementEarnedTrigger</code>.
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.update_achievement_progress_with_http_info(user_id, achievement_name, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method update_achievement_progress" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'user_id' is set
        if ('user_id' not in params) or (params['user_id'] is None):
            raise ValueError("Missing the required parameter `user_id` when calling `update_achievement_progress`")
        # verify the required parameter 'achievement_name' is set
        if ('achievement_name' not in params) or (params['achievement_name'] is None):
            raise ValueError("Missing the required parameter `achievement_name` when calling `update_achievement_progress`")
        ...
        return self.api_client.call_api('/users/{user_id}/achievements/{achievement_name}', 'PUT',
                                        path_params,
                                        query_params,
                                        header_params,
                                        body=body_params,
