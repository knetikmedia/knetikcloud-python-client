...
class StoreCouponsApi(object):
    ...
    def get_coupon_item_by_sku(self, sku, **kwargs):
        ...
        :param async bool
        :param str sku: A sku of the coupon (required)
        :return: CouponItem
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async'):
            return self.get_coupon_item_by_sku_with_http_info(sku, **kwargs)
        else:
            (data) = self.get_coupon_item_by_sku_with_http_info(sku, **kwargs)
            return data
    ...
    def get_coupon_item_by_sku_with_http_info(self, sku, **kwargs):
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_coupon_item_by_sku" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'sku' is set
        if ('sku' not in params) or (params['sku'] is None):
            raise ValueError("Missing the required parameter `sku` when calling `get_coupon_item_by_sku`")
        ...
        return self.api_client.call_api('/store/coupons/skus/{sku}', 'GET',
                                        path_params,
                                        query_params,
                                        header_params,
                                        body=body_params,
