...
class GamificationTriviaApi(object):
    ...
    def get_question_tags1(self, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_question_tags1(callback=callback_function)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.get_question_tags1_with_http_info(**kwargs)
        else:
            (data) = self.get_question_tags1_with_http_info(**kwargs)
            return data

    def get_question_tags1_with_http_info(self, **kwargs):
        """
        List and search tags by the beginning of the string
        For performance reasons, search & category filters are mutually exclusive. If category is specified, search filter will be ignored in order to do fast matches for typeahead.
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_question_tags1_with_http_info(callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_question_tags1" % key
                )
            params[key] = val
        del params['kwargs']
