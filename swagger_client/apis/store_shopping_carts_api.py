...
class StoreShoppingCartsApi(object):
    ...
    def get_countries(self, id, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_countries(id, callback=callback_function)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.get_countries_with_http_info(id, **kwargs)
        else:
            (data) = self.get_countries_with_http_info(id, **kwargs)
            return data

    def get_countries_with_http_info(self, id, **kwargs):
        """
        Get the list of available shipping countries per vendor
        Since a cart can have multiple vendors with different shipping options, the countries are broken down by vendors. Please see notes about the response object as the fields are variable.
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_countries_with_http_info(id, callback=callback_function)
        ...
        # verify the required parameter 'id' is set
        if ('id' not in params) or (params['id'] is None):
            raise ValueError("Missing the required parameter `id` when calling `get_countries`")
