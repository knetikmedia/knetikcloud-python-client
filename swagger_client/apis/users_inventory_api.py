...
class UsersInventoryApi(object):
    ...
    def get_user_inventory1(self, id, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_user_inventory1(id, callback=callback_function)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.get_user_inventory1_with_http_info(id, **kwargs)
        else:
            (data) = self.get_user_inventory1_with_http_info(id, **kwargs)
            return data

    def get_user_inventory1_with_http_info(self, id, **kwargs):
        """
        List the user inventory entries for a given user
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_user_inventory1_with_http_info(id, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_user_inventory1" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'id' is set
        if ('id' not in params) or (params['id'] is None):
            raise ValueError("Missing the required parameter `id` when calling `get_user_inventory1`")
    ...
    def get_user_inventory_log1(self, user_id, id, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_user_inventory_log1(user_id, id, callback=callback_function)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.get_user_inventory_log1_with_http_info(user_id, id, **kwargs)
        else:
            (data) = self.get_user_inventory_log1_with_http_info(user_id, id, **kwargs)
            return data

    def get_user_inventory_log1_with_http_info(self, user_id, id, **kwargs):
        """
        Set the expiration date
        Will change the current grace period for a subscription but not the bill date (possibly even ending before having the chance to re-bill)
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_user_inventory_log1_with_http_info(user_id, id, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_user_inventory_log1" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'user_id' is set
        if ('user_id' not in params) or (params['user_id'] is None):
            raise ValueError("Missing the required parameter `user_id` when calling `get_user_inventory_log1`")
        # verify the required parameter 'id' is set
        if ('id' not in params) or (params['id'] is None):
            raise ValueError("Missing the required parameter `id` when calling `get_user_inventory_log1`")
